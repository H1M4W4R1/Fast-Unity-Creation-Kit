...
namespace FastUnityCreationKit.Unity
{
        ...
        IMonoBehaviourSingleton<CKEventsManager> // Can't be FMB to prevent infinite loop
    {
        ...
        /// </summary>
        public static bool WasDestroyed { [UsedImplicitly] get; set; }
        ...
        {
            if (!_fastMonoBehaviours.Contains(ckMonoBehaviour)) _fastMonoBehaviours.Add(ckMonoBehaviour);
        }
        ...
        {
            if (_fastMonoBehaviours.Contains(ckMonoBehaviour)) _fastMonoBehaviours.Remove(ckMonoBehaviour);
        }
        ...
        private void ExecuteForAll(Action<CKMonoBehaviour, float> action, float deltaTime)
        {
                 ...
                 fastMonoBehaviourIndex++)
            {
                ...
                if (ckMonoBehaviour.IsDisabled &&
                    (ckMonoBehaviour.UpdateMode & UpdateMode.UpdateWhenDisabled) == 0)
                    continue;
