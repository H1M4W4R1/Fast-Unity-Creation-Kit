...
namespace FastUnityCreationKit.UI.Abstract
{
    ...
    public sealed class WindowStack
    {
        ...
        /// </summary>
        [NotNull] internal List<UIWindow> Windows { get; } = new();
        ...
        internal void Sort()
        {
            ...
            for (int i = 0; i < Windows.Count; i++) Windows[i].SetOrder(StartOrder + i);
        }
        ...
        /// </summary>
        public bool HasWindow<TWindowType>()
            where TWindowType : UIWindow
        {
            for (int i = 0; i < Windows.Count; i++)
                if (Windows[i] is TWindowType)
                    return true;
        ...
        [CanBeNull] public TWindowType GetWindow<TWindowType>()
            where TWindowType : UIWindow
        {
            for (int i = 0; i < Windows.Count; i++)
                if (Windows[i] is TWindowType window)
                    return window;
