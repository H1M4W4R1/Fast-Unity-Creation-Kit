...
namespace FastUnityCreationKit.UI.Context.Providers.Base
{
    ...
    public abstract class ListElementContextProviderBase<TContextType> : DataContextProviderBase<TContextType>
    {
        ...
        public override TContextType Provide()
        {
            ...
            ListContextProviderBase<TContextType> listProvider =
                GetComponentInParent<ListContextProviderBase<TContextType>>();

            // Ensure that the list provider is not null
            if (listProvider == null)
            {
                Guard<ValidationLogConfig>.Error(
                    $"ListElementContextProviderBase requires a ListContextProviderBase in the parent on object {name}.");
                return default;
            }
            ...
            {
                Guard<ValidationLogConfig>.Error(
                    $"Index {Index} is out of bounds for ListElementContextProviderBase on object {name}.");
                return default;
            }
