...
namespace FastUnityCreationKit.UI.Features.Snapping
{
        ...
        where TSnapObject : UIObjectBase, ISnapTarget<TSnapObject>
    {
        ...
        /// </summary>
        [ShowInInspector] [ReadOnly] [TitleGroup(GROUP_CONFIGURATION)] protected virtual bool UseMousePosition
            => true;

        [ShowInInspector] [ReadOnly] [TitleGroup(GROUP_CONFIGURATION)] protected virtual bool StartSnapped
            => false;

        private void Start()
        {
            // Snap to the closest object after everything is initialized
            if (StartSnapped) ExecuteSnap(transform.position);
        }
        ...
        public void OnPointerUp(PointerEventData eventData)
        {
            ...
            if (_searchForSnapActive) ExecuteSnap(UseMousePosition ? eventData.position : transform.position);
        }
        ...
        /// </summary>
        /// <param name="position">
        ///     Position from which you would like to search for TSnapObject (targets)
        ///     this object can snap to.
        /// </param>
        ...
        private void NotifyObjectSnapped([CanBeNull] TSnapObject toObject)
        {
            ...
            BroadcastMessage(nameof(ISnapCallback<TSnapObject>.OnSnap), toObject,
                SendMessageOptions.DontRequireReceiver);

            // And notify target slot that it was snapped
            toObject.SendMessage(nameof(IOnObjectSnappedCallback<TSnapObject>.OnSnap), this,
                SendMessageOptions.DontRequireReceiver);
        }
        ...
        private void NotifyObjectSnapBroken([CanBeNull] TSnapObject fromObject)
        {
            ...
            // Send messages to all components that implement ISnapCallback
            BroadcastMessage(nameof(ISnapCallback<TSnapObject>.OnSnapBreak), fromObject,
                SendMessageOptions.DontRequireReceiver);

            // And notify target slot that it was snapped
            fromObject.SendMessage(nameof(IOnObjectSnappedCallback<TSnapObject>.OnSnapBreak), this,
                SendMessageOptions.DontRequireReceiver);
        }
    }
}