...
namespace FastUnityCreationKit.Economy.Abstract
{
        ...
        where TNumberType : struct, INumber, ISupportsFloatConversion<TNumberType>
    {
        ...
        void IResource.AddValue(IAddResourceContext<float32> context) =>
            Add(new GenericAddResourceContext<TNumberType>(context.Economy, new TNumberType().FromFloat(context.Amount)));

        void IResource.TakeValue(ITakeResourceContext<float32> context) =>
            Take(new GenericTakeResourceContext<TNumberType>(context.Economy, new TNumberType().FromFloat(context.Amount)));

        bool IResource.TryTakeValue(ITakeResourceContext<float32> context) =>
            TryTake(new GenericTakeResourceContext<TNumberType>(context.Economy, new TNumberType().FromFloat(context.Amount)));

        bool IResource.HasEnoughValue(ICompareResourceContext<float32> context) =>
            HasEnough(new GenericCompareResourceContext<TNumberType>(context.Economy, new TNumberType().FromFloat(context.Amount)));

        void IResource.SetValue(IModifyResourceContext<float32> context)
            => SetAmount(new GenericModifyResourceContext<TNumberType>(context.Economy, new TNumberType().FromFloat(context.Amount)));
    }
    ...
    {
        internal void AddValue([NotNull] IAddResourceContext<float32> context);

        internal void AddValue<TNumberType>([NotNull] IAddResourceContext<TNumberType> context)
            where TNumberType : struct, INumber, ISupportsFloatConversion<TNumberType>;

        internal void TakeValue([NotNull] ITakeResourceContext<float32> context);

        internal void TakeValue<TNumberType>([NotNull] ITakeResourceContext<TNumberType> context)
            where TNumberType : struct, INumber, ISupportsFloatConversion<TNumberType>;

        internal bool TryTakeValue([NotNull] ITakeResourceContext<float32> context);

        internal bool TryTakeValue<TNumberType>([NotNull] ITakeResourceContext<TNumberType> context)
            where TNumberType : struct, INumber, ISupportsFloatConversion<TNumberType>;

        internal bool HasEnoughValue([NotNull] ICompareResourceContext<float32> context);

        internal bool HasEnoughValue<TNumberType>([NotNull] ICompareResourceContext<TNumberType> context)
            where TNumberType : struct, INumber, ISupportsFloatConversion<TNumberType>;

        internal void SetValue([NotNull] IModifyResourceContext<float32> context);
