...
namespace FastUnityCreationKit.Editor.Postprocessing.Annotations
{
    ...
    [UsedImplicitly]
    public sealed class AutoCreatedObjectAttributeProcessor :
        QuickAssetProcessor<AutoCreatedObjectAttributeProcessor>,
        IPostprocessAllAssets
    {
        ...
        public void PostprocessAllAssets(
            ...
            [NotNull] string[] movedFromAssetPaths)
        {
            ...
            int totalCount = CountScripts(importedAssets) + CountScripts(deletedAssets) +
                             CountScripts(movedAssets);
        ...
        internal static void PerformAutoCreateObjectProcessing()
        {
            ...
            foreach (Assembly assembly in assemblies) ProcessAssembly(assembly);
        }
        ...
        private static void ProcessAssembly([NotNull] Assembly assembly)
        {
            ...
            if (createdCount > 0)
                Guard<ValidationLogConfig>.Debug(
                    $"Created {createdCount} assets from assembly {assembly.FullName}.");
        ...
        [NotNull] private static string ValidateAndGetDirectoryPath([NotNull] AutoCreatedObjectAttribute attribute)
        {
            ...
            if (!AssetDatabase.IsValidFolder(ROOT_PATH)) AssetDatabase.CreateFolder("Assets", "AutoGenerated");
        ...
        {
            return AssetDatabase.LoadAssetAtPath($"{path}{NameOf(type)}.asset", type) != null;
        }

        private static string NameOf(Type type)
        {
            return type.GetCompilableNiceFullName();
        }
    }
}