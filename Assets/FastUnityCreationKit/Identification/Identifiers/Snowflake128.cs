...
namespace FastUnityCreationKit.Identification.Identifiers
{
    ...
    public readonly struct Snowflake128 : IUniqueIdentifier, IEquatable<Snowflake128>
    {
       ...
       public string GetDebugTooltipText()
       {
           ...
           tooltipBuilder.AppendLine($"<color=#00FFFF>Creation date [UTC]:</color> {new DateTime(timestamp):yyyy-MM-dd HH:mm:ss}");
           ...
           tooltipBuilder.Append($"<color=#00FFFF>Is created:</color> {(created > 0 ? "<color=green>Yes</color>" : "<color=red>No</color>")}");
